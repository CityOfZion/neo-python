...
from neo.Network.neonetwork.core.mixin.serializable import SerializableMixin
from neo.Network.neonetwork.core.uintbase import UIntBase
    ...
    # internal static int GetVarSize<T>(this T[] value)
        ...
        if value_length > 0:
            if isinstance(value[0], SerializableMixin):
                ...
                raise TypeError(
                    "Can not accurately determine size of objects that do not inherit from 'SerializableMixin', 'Enum' or 'bytes'. Found type: {}".format(
                        type(value[0])))

    else:
        raise ValueError("[NOT SUPPORTED] Unexpected value type {} for GetVarSize()".format(type(value)))
