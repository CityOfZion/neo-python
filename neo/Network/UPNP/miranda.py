...
class upnp:
    ...
    def initSockets(self, ip, port, iface):
        ...
        try:
            ...
            if self.IFACE != None:
                ...
                self.ssock.setsockopt(SOL_SOCKET, IN.SO_BINDTODEVICE, struct.pack(
                    "%ds" % (len(self.IFACE) + 1,), self.IFACE))
                self.csock.setsockopt(SOL_SOCKET, IN.SO_BINDTODEVICE, struct.pack(
                    "%ds" % (len(self.IFACE) + 1,), self.IFACE))
    ...
    def send(self, data, socket):
        ...
        if socket == False:
            socket = self.csock
        try:
            socket.sendto(data.encode('utf-8'), (self.ip, self.port))
            return True
        except Exception as e:
    ...
    def parseSSDPInfo(self, data, showUniq, verbose):
        ...
        if messageType != False:
            ...
            if (hostFound and not self.UNIQ) or not hostFound:
                ...
                # Be sure to update the command completer so we can tab
                # complete through this host's data structure
        ...
        for device in xmlRoot.getElementsByTagName(devTag):
            ...
            for tag in deviceTags:
                try:
                    ...
                    if self.VERBOSE:
                        print('Device', deviceEntryPointer[
                              'fullName'], 'does not have a', tag)
                    continue
    ...
    def parseServiceInfo(self, service, index):
        ...
        xmlFile = self.ENUM_HOSTS[index][
            'proto'] + self.ENUM_HOSTS[index]['name']
        ...
        for var in stateVars:
            for tag in varVals:
                ...
                try:
                    ...
                    for val in vals:
                        servicePointer['serviceStateVariables'][varName][
                            allowedValueList].append(str(val.childNodes[0].data))
        ...
        if action == 'uniq':
            ...
            if argc == 3:
                ...
                if hp.initSockets(hp.ip, hp.port, hp.IFACE):
                    ...
                    print(
                        'Failed to bind new interface - are you sure you have root privilages??')
                    hp.IFACE = None
                return
        elif action == 'socket':
            if argc == 3:
        ...
        if action == 'list':
            ...
            if argc == 3:
                ...
                if hostInfo is not None:
                    ...
                    try:
                        ...
                        if hostInfo != False:
                            print("Requesting device and service info for %s (this could take a few seconds)..." % hostInfo[
                                  'name'])
                            print('')
                            if hostInfo['dataComplete'] == False:
            ...
            if argc != 6:
                ...
                try:
                    ...
                    controlURL2 = hostInfo['deviceList'][deviceName][
                        'services'][serviceName]['controlURL']
                ...
                # Get action info
                try:
                    actionArgs = hostInfo['deviceList'][deviceName]['services'][
                        serviceName]['actions'][actionName]['arguments']
                ...
                for argName, argVals in actionArgs.items():
                    ...
                    stateVar = hostInfo['deviceList'][deviceName]['services'][
                        serviceName]['serviceStateVariables'][actionStateVar]
                    ...
                    if argVals['direction'].lower() == 'in':
                        ...
                        if 'allowedValueRange' in stateVar:
                            print("\tValue Min:     ", stateVar[
                                  'allowedValueRange'][0])
                            print("\tValue Max:     ", stateVar[
                                  'allowedValueRange'][1])
...
def main(argc, argv):
    ...
    # The load command should auto complete on the contents of the current
    # directory
    ...
    while True:
        ...
        if isinstance(funcPtr, collections.Callable):
            if argc == 2 and argv[1] == 'help':
                showHelp(argv[0])
            else:
                try:
